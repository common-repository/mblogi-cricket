    ...
    var methods = {
        ...
        init: function (options) {
            ...
            return this.each(function () { //uncomment this to allow multiple instances on one constructor call
                var cookieName = 'accordion_' + $(this).attr('id');
                ...
                if (options.useCookies) {
                    ...
                    if (c != null) {
                        $(c.split(' ')).each(function (i, e) {
                            cookiePositions.push(parseInt(e));
                        });
                    }
                ...
                $("li", $menu).each(function () {
                    if ($.inArray(liNumId, cookiePositions) != -1) {
                        $(this).find('li:first').addClass(options.defaultExpanded.replace(".", ""));
                    }
                ...
                if (options.useCookies) {
                    $("a", $menu).bind(options.event, function () {
                        var c = '';
                        $('li.' + options.expandedPathClass).each(function (i, e) {
                            var data = $(e).data('ctAccordion');
                            c += ' ' + data.numID;
                        });

                        setCookie(cookieName, c.trim());
                    });
        ...
        search: function (query) {
            var $menu = $(this), data = $menu.data("ctAccordion"), options = data.options, regex = new RegExp(query, "i");


            hideUnused.apply($menu);

            $("li>a, >li>ul>li", $menu).each(function () {
        ...
        open: function (level1Index, level2Index) {
            var $menu = $(this), data = $menu.data("ctAccordion"), options = data.options;
        ...
        option: function (key, value) {
            if (typeof key === "string") {
                if (value === undefined) {
                    // behave as a "getter"
                    var $container = $(this), data = $container.data("ctAccordion");
    ...
    var hideUnused = function ($el) {
        ...
        if ($el === undefined) {
            ...
            $("ul:visible", $menu).each(function (i, ul) {
                var $ul = $(ul), data = $ul.data("ctAccordion");
